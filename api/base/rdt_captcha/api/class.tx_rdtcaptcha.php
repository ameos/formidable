 ...
 */
class tx_rdtcaptcha extends formidable_mainrenderlet {
	...
	function _render() {
		...
		if($this->_navConf("/reloadpic")) {
			if(is_string(trim(strtolower($this->aElement["reloadpic"])))) {
		...
		$sCopy = $this->oForm->_getLLLabel($this->aElement["copylabel"]);
		...
		$this->includeScripts(
			array(
				"reloadurl" => $this->oForm->toWebPath(
		...
		$aHtmlBag = array(
			"__compiled" => $this->_displayLabel($this->getLabel()) .$aCaptcha["img"].$aCaptcha["reload"]."<br />".$sCopy."<br />". $sInput,
			"image"=>$aCaptcha["img"],
	...
	function declareCustomValidationErrors() {
		if(
			...
			$this->oForm->oDataHandler->_isTestSubmitted() || (
				$this->oForm->oDataHandler->_isDraftSubmitted() &&
				$this->_hasToValidateForDraft()
			)
		) {
...
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/ameos_formidable/api/base/rdt_captcha/api/class.tx_rdtcaptcha.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/ameos_formidable/api/base/rdt_captcha/api/class.tx_rdtcaptcha.php"]);
}
